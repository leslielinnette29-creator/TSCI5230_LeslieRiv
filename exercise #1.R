...
patients %>%mutate(DEATHDATE=as.Date(DEATHDATE),BIRTHDATE=as.Date(BIRTHDATE),alive=is.na(DEATHDATE),
                   Enddate=pmin(Sys.Date(),DEATHDATE,na.rm = TRUE),age=as.numeric(Enddate-BIRTHDATE)/365.25) %>% 
  group_by(alive) %>% 
  summarize(
...
>>>>>>> be84cfa4a27131216053b611644d07e675cd41e2
